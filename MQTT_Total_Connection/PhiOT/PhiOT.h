...
class PhiOT
{
  public:
    ...
    void Initialize();
    void phiLoop();
    void Phicallback(char* topic, byte* payload, unsigned int length);
    void reconnect();
    void mqttInit();
    ...
    PhiOT& setServer(const char * domain, uint16_t port);
    PhiOT& setCallback(MQTT_CALLBACK_SIGNATURE);
    PhiOT& setClient(Client& client);
